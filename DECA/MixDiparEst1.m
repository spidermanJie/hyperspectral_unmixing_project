function [a_hat,e_hat,b,L_tot,e_hat_tot,kill_mode]=MixDiparEst1(s_hat,Q,th,niter_max,a_hat,e_hat,verbose,ver_fig)
...
while ~stop_while_Lfunc  & (iter < niter_max) & ~stop_while_modezero
    ...
    a_hat = psi_inv( repmat(psi(sum(a_hat)+realmin),[p 1]) + mbx + (dddd-1)./a_hat./repmat(sum(b)+realmin,[p 1]) );
    % Equation (17)
...
function param_est = est_dirichlet_param1(x,verbose)
